...
test_that("tabyl::chisq.test on a table is correct", {
  res <- stats::chisq.test(tab)
  jres <- tabyl::chisq.test(tab)
  expect_equal(jres, res)
})

test_that("tabyl::chisq.test on a matrix is correct", {
  mat <- matrix(c(151, 434, 345, 221, 145, 167), ncol = 3)
  res <- stats::chisq.test(mat)
  jres <- tabyl::chisq.test(mat)
  expect_equal(jres, res)
})

test_that("tabyl::chisq.test on two factors is correct", {
  res <- stats::chisq.test(mtcars3$am, mtcars3$cyl)
  jres <- tabyl::chisq.test(mtcars3$am, mtcars3$cyl)
  expect_equal(jres, res)
})

test_that("tabyl::chisq.test with a numeric vector and p is correct", {
  v1 <- round(runif(10, 200, 1000))
  v2 <- round(runif(10, 200, 1000))
  res <- stats::chisq.test(v1, p = v2 / sum(v2))
  jres <- tabyl::chisq.test(v1, p = v2 / sum(v2))
  expect_equal(jres, res)
})

test_that("tabyl::fisher.test on a table is correct", {
  res <- stats::fisher.test(tab)
  jres <- tabyl::fisher.test(tab)
  expect_equal(jres, res)
})

test_that("tabyl::fisher.test on a matrix is correct", {
  mat <- matrix(c(151, 434, 345, 221, 145, 167), ncol = 3)
  res <- stats::fisher.test(mat)
  jres <- tabyl::fisher.test(mat)
  expect_equal(jres, res)
})

test_that("tabyl::fisher.test on two vectors is correct", {
  res <- stats::fisher.test(mtcars3$am, mtcars3$cyl)
  jres <- tabyl::fisher.test(mtcars3$am, mtcars3$cyl)
  expect_equal(jres, res)
})

test_that("tabyl::chisq.test on a two-way tabyl is identical to stats::chisq.test", {
...
test_that("tabyl::fisher.test on a two-way tabyl is identical to stats::fisher.test", {
  tab <- tabyl(mtcars3, am, cyl)
...
test_that("tabyl::chisq.test returns tabyl tables", {
