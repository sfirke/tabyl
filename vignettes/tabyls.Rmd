...
+ **`adorn_rounding()`**: Round a data.frame of numbers (usually the result of `adorn_percentages`), either using the base R `round()` function or using tabyl's `tabyl::round_half_up()` to round all ties up ([thanks, StackOverflow](https://stackoverflow.com/a/12688836/4470365)).
...
File [an issue on GitHub](https://github.com/sfirke/tabyl/issues) if you have suggestions related to `tabyl()` and its `adorn_` helpers or encounter problems while using them.
